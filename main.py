...
def createWidgets(root, top):
    ...
    # Question label
    root.label_question = ttk.Label(textvariable=root.question, font=("Aptos ExtraBold",15), wraplength=500, anchor="center")
    root.label_question.grid(column=1, row=2, columnspan=5, pady=10)

    # Progress label
    root.label_progress = ttk.Label(textvariable=root.ProgressText, font=("Arial", 12))
    root.label_progress.grid(column=1, row=1, columnspan=5)

    # Score label
    root.label_score = ttk.Label(text="Score:", font=("Arial", 14))
    root.label_score.grid(column=4, row=3, sticky="e")
    root.label_score_value = ttk.Label(textvariable=root.score, font=("Arial", 12))
...
#Create multiple choices
    root.Button1 = ttk.Radiobutton(root,style='info.Outline.Toolbutton', textvariable=root.O1, variable=root.user_selected, value=1, command=lambda:set_choices(1))
    root.Button2 = ttk.Radiobutton(root,style='info.Outline.Toolbutton', textvariable=root.O2, variable=root.user_selected, value=2, command=lambda:set_choices(2))
    root.Button3 = ttk.Radiobutton(root,style='info.Outline.Toolbutton',textvariable=root.O3, variable=root.user_selected, value=3, command=lambda:set_choices(3))
    root.Button4 = ttk.Radiobutton(root,style='info.Outline.Toolbutton', textvariable=root.O4,variable=root.user_selected, value=4, command=lambda:set_choices(4))
